...
function zrdn_editor_assets() {
	...
	wp_enqueue_script(
        ...
        filemtime( plugin_dir_path( __DIR__ ) . 'build/blocks.js' ), // Version: File modification time.
    ...
    wp_localize_script(
        ...
        array(
			...
			'active_blocks' => ZipRecipes::get_all_usable_blocks(),
        )
    );
    ...
    wp_enqueue_style(
        'zrdn-block', // Handle.
        trailingslashit(ZRDN_PLUGIN_URL) . "build/blocks.css", array( 'wp-edit-blocks' ), ZRDN_VERSION_NUM
    );
...
function zrdn_render_block_nutrition_label($attributes)
{
	$html = __('Please select a recipe', 'zip-recipes');
	if (isset($attributes['recipe_id']) && absint($attributes['recipe_id'])) {
		$html = do_shortcode( '[zrdn-nutrition-label recipe_id=' . absint($attributes['recipe_id']) . ']');
	}
...
register_block_type('zip-recipes/nutrition-label', array(
	'attributes' => [
		'recipe_id' => [
			'type' => 'string',
            'default' =>  '',
	...
	'render_callback' => __NAMESPACE__ . '\zrdn_render_block_nutrition_label',
));
