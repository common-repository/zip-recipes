...
//	    self::$recipe_sharing = new ZRDN_recipe_sharing_admin();
	...
	public static function get_all_usable_blocks() {
        $available_blocks = array_filter( self::all_available_blocks(), function( $item ) {
            if ( ! empty( $item['premium'] ) && ! ZipRecipes::is_paying_user() ) {
                return false;
            }
            return true;
        } );

        $not_data_blocks = array(
            'divider',
            'nutrition_label'
        );
        ...
        foreach ( $available_blocks as $block ) {
            if ( in_array( $block['type'], $not_data_blocks, true ) ) {
                continue;
            }
	...
	public static function is_paying_user() {

		$license_valid = false;

		if ( defined('ZRDN_PREMIUM') ){
            if ( ! class_exists( '\ZRDN\licensing' ) ) {
                require_once trailingslashit( ZRDN_PATH ) . 'plugins/Licensing/Licensing.php';
            }
